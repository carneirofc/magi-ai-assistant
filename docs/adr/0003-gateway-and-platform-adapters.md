...
5. **Two real implementers, not a Protocol with one user.** `DiscordClient`
   satisfies `PlatformAdapter` structurally for free (it already had
   `serve_async`; it gains a `platform = "discord"` class attribute). The API
   channel gains a small, additive `ApiAdapter` / `build_api_adapter()`
...
- `clients/mydiscord.py`'s three `self.conversation.*` call sites (`handle`,
  ...
  message_user_id)` instead of the raw snowflake.
