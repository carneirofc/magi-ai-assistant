...
def no_local_data(monkeypatch):
    ...
    configure(danbooru_tags_csv="missing/tags.csv", danbooru_wiki_csv="missing/wiki.csv")
    yield
...
async def test_429_backs_off_per_retry_after_then_succeeds(monkeypatch):
...
async def test_second_429_returns_error_line(monkeypatch):
    _patch_client(
        monkeypatch,
...
async def test_search_artists_filters_category_and_hits_live_api(monkeypatch):
    ...
    result = await danbooru.danbooru_search_artists.entrypoint(query="WLOP")
