...
export async function UserMemoryView({
  params,
  copy,
}: {
  ...
  const header = mergeCopy(
    { subtitle: "magi // memory", title: userId, description: "Durable memory for this user." },
    copy,
  );
...
export default function UserMemoryPage({ params }: { params: Promise<{ user: string }> }) {
  return <UserMemoryView params={params} />;
}
