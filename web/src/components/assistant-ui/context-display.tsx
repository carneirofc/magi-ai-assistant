...
// assistant-ui's ContextDisplay component (docs: /docs/ui/context-display),
// copied in as a project-owned file (the shadcn distribution model) and adapted
// to this app in two ways, without changing its structure, math, or API:
//
//   1. Runtime source — upstream reads token usage from the AI-SDK runtime via
//      `useThreadTokenUsage()`. This console runs a custom LocalRuntime + SSE
//      adapter, not the AI SDK, so we drive it through the component's built-in
//      `usage` prop (its sanctioned escape hatch) and drop the AI-SDK import; the
//      composer maps our streamed `ChatUsage` into `ThreadTokenUsage` and passes
//      it in. See ChatConsole.tsx / chat-adapter.ts.
//   2. Theming — upstream uses shadcn semantic classes (bg-muted, bg-popover,
//      text-muted-foreground, …) that don't exist in this Tailwind config, so the
//      neutral chrome is re-tokenised to @carneirofc/ui vars. The emerald/amber/
//      red severity colors are kept as-is — they're the component's identity.
//
// `ContextDisplay.Bar` / `.Ring` / `.Text` render a usage indicator with a
// hover tooltip breaking the turn down (input/cached/output/reasoning/total).
    ...
    >
      <div className="grid min-w-40 gap-1.5 text-xs">
        <div className="flex items-center justify-between gap-4">
          <span className="text-[color:var(--ui-ink-subtle)]">Usage</span>
        ...
        {usage?.reasoningTokens !== undefined && usage.reasoningTokens > 0 && (
          <div className="flex items-center justify-between gap-4">
            <span className="text-[color:var(--ui-ink-subtle)]">Reasoning</span>
            <span className="font-mono tabular-nums">
              {formatTokenCount(usage.reasoningTokens)}
            </span>
          </div>
        )}
        <div className="mt-0.5 border-t border-ui pt-1.5">
          <div className="flex items-center justify-between gap-4">
  ...
  return (
    ...
    >
      <circle
        ...
        className="stroke-[color:var(--ui-bg-soft)]"
      />
      <circle
        cx={RING_SIZE / 2}
        cy={RING_SIZE / 2}
...
function BarVisual() {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] tabular-nums text-[color:var(--ui-ink-subtle)]">
...
const ContextDisplayText: FC<PresetProps> = ({
  ...
  <ContextDisplayRoot modelContextWindow={modelContextWindow} usage={usage}>
    <ContextDisplayTrigger
      aria-label="Context usage"
      className={cn(
        "px-2 py-1 font-mono text-xs tabular-nums text-[color:var(--ui-ink-subtle)] hover:bg-[color:var(--ui-bg-soft)] hover:text-[color:var(--ui-ink)]",
        className,
      )}
    >
      <TextVisual />
